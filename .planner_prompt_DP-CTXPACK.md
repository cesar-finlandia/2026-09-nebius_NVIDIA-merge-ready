...
- Design plan (READ IN FULL): `design_documents/design_plans/DP-CTXPACK.md`
- Output path (WRITE THIS FILE, overwrite if present): `.run_sweep/prompts_DP-CTXPACK.json`
- Module id: `DP-CTXPACK`
- Models / roles file (READ): `models.json`

`.run_sweep/prompts_DP-CTXPACK.json` does not exist yet and MUST NOT be treated as an input. Do not
...
Write valid UTF-8 JSON to `.run_sweep/prompts_DP-CTXPACK.json` with this shape:

```json
{
  "module": "DP-CTXPACK",
  "module_name": "<short human name from the design plan title>",
  "design_plan": "design_documents/design_plans/DP-CTXPACK.md",
...
1. Read `design_documents/design_plans/DP-CTXPACK.md` thoroughly (do not skim).
...
5. Build `.run_sweep/prompts_DP-CTXPACK.json` INCREMENTALLY with MANY SMALL file operations -
   NEVER one giant write. Long single-shot generations get cut off by the
   provider mid-stream; many small writes are robust:
   a. First Write a minimal skeleton to `.run_sweep/prompts_DP-CTXPACK.json`:
      {{"module": "DP-CTXPACK", "prompts": [ __STEP__ ]}}
   ...
   d. When all steps are in, Edit away the trailing ", __STEP__" placeholder,
      ...
      parses as JSON (e.g. python3 -c "import json; json.load(open('.run_sweep/prompts_DP-CTXPACK.json'))").
...
- `.run_sweep/prompts_DP-CTXPACK.json` exists
- JSON parses
- `module` equals `DP-CTXPACK`
