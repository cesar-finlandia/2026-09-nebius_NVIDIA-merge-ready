...
- Design plan (READ IN FULL): `design_documents/design_plans/DP-PR.md`
- Output path (WRITE THIS FILE, overwrite if present): `.run_sweep/prompts_DP-PR.json`
- Module id: `DP-PR`
- Models / roles file (READ): `models.json`

`.run_sweep/prompts_DP-PR.json` does not exist yet and MUST NOT be treated as an input. Do not
...
Write valid UTF-8 JSON to `.run_sweep/prompts_DP-PR.json` with this shape:

```json
{
  "module": "DP-PR",
  "module_name": "<short human name from the design plan title>",
  "design_plan": "design_documents/design_plans/DP-PR.md",
...
1. Read `design_documents/design_plans/DP-PR.md` thoroughly (do not skim).
...
5. Build `.run_sweep/prompts_DP-PR.json` INCREMENTALLY with MANY SMALL file operations -
   NEVER one giant write. Long single-shot generations get cut off by the
   provider mid-stream; many small writes are robust:
   a. First Write a minimal skeleton to `.run_sweep/prompts_DP-PR.json`:
      {{"module": "DP-PR", "prompts": [ __STEP__ ]}}
   ...
   d. When all steps are in, Edit away the trailing ", __STEP__" placeholder,
      ...
      parses as JSON (e.g. python3 -c "import json; json.load(open('.run_sweep/prompts_DP-PR.json'))").
...
- `.run_sweep/prompts_DP-PR.json` exists
- JSON parses
- `module` equals `DP-PR`
