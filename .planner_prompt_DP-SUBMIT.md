...
- Design plan (READ IN FULL): `design_documents/design_plans/DP-SUBMIT.md`
- Output path (WRITE THIS FILE, overwrite if present): `.run_sweep/prompts_DP-SUBMIT.json`
- Module id: `DP-SUBMIT`
- Models / roles file (READ): `models.json`

`.run_sweep/prompts_DP-SUBMIT.json` does not exist yet and MUST NOT be treated as an input. Do not
...
Write valid UTF-8 JSON to `.run_sweep/prompts_DP-SUBMIT.json` with this shape:

```json
{
  "module": "DP-SUBMIT",
  "module_name": "<short human name from the design plan title>",
  "design_plan": "design_documents/design_plans/DP-SUBMIT.md",
...
1. Read `design_documents/design_plans/DP-SUBMIT.md` thoroughly (do not skim).
...
5. Build `.run_sweep/prompts_DP-SUBMIT.json` INCREMENTALLY with MANY SMALL file operations -
   NEVER one giant write. Long single-shot generations get cut off by the
   provider mid-stream; many small writes are robust:
   a. First Write a minimal skeleton to `.run_sweep/prompts_DP-SUBMIT.json`:
      {{"module": "DP-SUBMIT", "prompts": [ __STEP__ ]}}
   ...
   d. When all steps are in, Edit away the trailing ", __STEP__" placeholder,
      ...
      parses as JSON (e.g. python3 -c "import json; json.load(open('.run_sweep/prompts_DP-SUBMIT.json'))").
...
- `.run_sweep/prompts_DP-SUBMIT.json` exists
- JSON parses
- `module` equals `DP-SUBMIT`
